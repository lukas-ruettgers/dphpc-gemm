    ...
    int BM_, int BN_, int BK_)
{
    ...
    // Shared memory
    __shared__ alignas(16) float sA[2][TB_M_ * TB_K_];
    __shared__ alignas(16) float sB[2][TB_K_ * TB_N_];
...
int main(int argc, char** argv) {
    ...
    size_t smem_needed = 2*(TB_M * TB_K + TB_K * TB_N) * sizeof(float);
