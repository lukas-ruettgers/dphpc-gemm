...
namespace gemm_vectorized_loads {
...
__global__ void gemm_kernel_vectorized(half_t const* __restrict__ A,
                                       ...
                                       int lda, int ldb, int ldc)
{
  ...
  using CopyAtomA = Copy_Atom<AutoVectorizingCopyWithAssumedAlignment<128>, TA>;
  using CopyAtomB = Copy_Atom<AutoVectorizingCopyWithAssumedAlignment<128>, TB>;
...
template<int BM=128, int BN=64, int BK=32>
inline void gemm_vectorized_launch(const half_t *dA,
                                   const half_t *dB,
                                   float *dC,
                                   int M, int N, int K,
                                   int lda, int ldb, int ldc,
                                   cudaStream_t stream = 0)
{
  ...
  cudaFuncSetAttribute(
      gemm_kernel_vectorized<BM,BN,BK>,
      cudaFuncAttributeMaxDynamicSharedMemorySize,
      (int)smem_bytes);

  gemm_kernel_vectorized<BM,BN,BK><<<grid, block, smem_bytes, stream>>>(
