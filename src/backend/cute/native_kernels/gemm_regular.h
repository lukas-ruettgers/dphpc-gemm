...
__global__ void gemm_kernel_scalar(half_t const* __restrict__ A,
                                   ...
                                   int lda, int ldb, int ldc)
{
  ...
  // Use uint16_t (2 bytes) to force scalar 16-bit loads for half_t
  using CopyAtomA = Copy_Atom<UniversalCopy<uint16_t>, TA>;
  using CopyAtomB = Copy_Atom<UniversalCopy<uint16_t>, TB>;
  ...
  {
    // Scalar synchronous copies (will use LDG.16 instructions)
...
inline void gemm_scalar_launch(const half_t *dA,
                               const half_t *dB,
                               float *dC,
                               int M, int N, int K,
                               int lda, int ldb, int ldc,
                               cudaStream_t stream = 0)
{
  ...
  constexpr int PAD_A = 8;
  constexpr int PAD_B = 8;
  size_t smem_bytes = (BM * (BK + PAD_A) + BN * (BK + PAD_B)) * sizeof(half_t);

  cudaFuncSetAttribute(
      gemm_kernel_scalar<BM,BN,BK>,
      cudaFuncAttributeMaxDynamicSharedMemorySize,
      (int)smem_bytes);

  gemm_kernel_scalar<BM,BN,BK><<<grid, block, smem_bytes, stream>>>(
