...
def parse_results(filename):
    ...
    # --- BN=1, n=128 ---
    re_bn = re.compile(r"^---\s*BN=(\d+),\s*n=(\d+)\s*---")
...
def plot_results(bn, n_values, runtimes, gflops, out_prefix="streamk_analysis"):
    ...
    print(f"Saved plots to: {out_prefix}_gflops.png and {out_prefix}_runtime.png")
...
def main():
    ...
    bn, n_values, runtimes, gflops = parse_results(logfile)

    # Just to show the array shapes and an example:
    print(f"Parsed {len(bn)} BN points")
    for key in sorted(runtimes.keys()):
        print(
            f"{key:20s}: runtimes shape={runtimes[key].shape}, "
            f"GFLOPs shape={gflops[key].shape}"
        )

    # Create plots (you can comment this out if you only want the arrays)
    out_prefix = logfile.with_suffix("").as_posix()
    plot_results(bn, n_values, runtimes, gflops, out_prefix=out_prefix)
